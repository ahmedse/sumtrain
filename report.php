<?php
    ...
    $url = new moodle_url('/mod/sumtrain/report.php', array('id'=>$id));
    if ($download !== '') {
    ...
    require_capability('mod/sumtrain:readresponses', $context);
    ...
    $strresponses = get_string("responses", "sumtrain");
    ...
    if (data_submitted() && has_capability('mod/sumtrain:deleteresponses', $context) && confirm_sesskey()) {
        if ($action === 'delete') {
    ...
    if (!$download) {
        $PAGE->set_title(format_string($sumtrain->name).": $strresponses");
        ...
        if ($groupmode) {
            ...
            $groupsactivitymenu = groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/sumtrain/report.php?id=' . $id,
                true);
    ...
    if ($download == "ods" && has_capability('mod/sumtrain:downloadresponses', $context)) {
        require_once("$CFG->libdir/odslib.class.php");

    /// Calculate file name
        $shortname = format_string($course->shortname, true, array('context' => $context));
        $sumtrainname = format_string($sumtrain->name, true, array('context' => $context));
        $filename = clean_filename("$shortname " . strip_tags($sumtrainname)) . '.ods';
    /// Creating a workbook
    ...
    /// Print names of all the fields
        ...
        $myxls->write_string(0, $i++, get_string("sumtrain", "sumtrain"));

        // Generate the data for the body of the spreadsheet.
        $row = 1;
        if ($users) {
            foreach ($users as $option => $userid) {
                if (!($sumtrain->showunanswered  == 0 && $option == 0)) {
                    $option_text = sumtrain_get_option_text($sumtrain, $option);
    ...
    if ($download == "xls" && has_capability('mod/sumtrain:downloadresponses', $context)) {
        require_once("$CFG->libdir/excellib.class.php");

    /// Calculate file name
        $shortname = format_string($course->shortname, true, array('context' => $context));
        $sumtrainname = format_string($sumtrain->name, true, array('context' => $context));
        $filename = clean_filename("$shortname " . strip_tags($sumtrainname)) . '.xls';
    /// Creating a workbook
    ...
    // print text file
    if ($download == "txt" && has_capability('mod/sumtrain:downloadresponses', $context)) {
        $shortname = format_string($course->shortname, true, array('context' => $context));
        $sumtrainname = format_string($sumtrain->name, true, array('context' => $context));
        $filename = clean_filename("$shortname " . strip_tags($sumtrainname)) . '.txt';
        ...
        echo get_string("sumtrain","sumtrain"). "\n";
   ...
   //now give links for downloading spreadsheets.
    if (!empty($users) && has_capability('mod/sumtrain:downloadresponses',$context)) {
